      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-6">
            {(Array.isArray(orders) ? orders : []).map((order) => (
              <div key={order?.id || Math.random()} className="bg-white rounded-lg shadow p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-semibold">شماره سفارش: {order?.order_number || ''}</h3>
                    <p className="text-gray-500 text-sm">تاریخ: {order?.order_date ? new Date(order.order_date).toLocaleDateString('fa-IR') : ''}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p>{(order?.shipping_cost || 0).toLocaleString()} تومان</p>
                  </div>
                  <div>
                    <span className="text-gray-500">مبلغ کل:</span>
                    <p className="font-semibold">{(order?.total_price || 0).toLocaleString()} تومان</p>
                  </div>
                  {order?.address && (
                    <div>
