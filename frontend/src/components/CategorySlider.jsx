    ...
    <section data-animate className="mb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {categories.map((c, i) => (
          ...
          >
            <div className={`w-14 h-14 ${c.color} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform`}>
              <DynamicIcon name={c.icon} className="w-7 h-7" />
            </div>
            <span className="font-semibold text-sm text-center">{c.name}</span>
            <span className="text-xs text-gray-400">{c.productCount} items</span>
          </Link>
        ))}
      </div>
    </section>
